...
function MinutePulse({ minute, status }: { minute: number | string; status: string }) {
  ...
  if (status === "ht") {
    return <span className="text-[11px] font-bold text-[var(--color-warning)]">HT</span>;
  }
  if (status === "upcoming") {
...
function OddsButton({
  ...
  return (
    ...
    >
      <span className={`text-[9px] block ${isSelected ? "text-[#0C1220]/70" : "text-[var(--color-text-dim)]"}`}>{label}</span>
      <span className={`text-xs font-bold ${isSelected ? "text-[#0C1220]" : "text-[var(--color-accent-primary)]"}`}>{value}</span>
    </button>
  );
}
          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                filter === f.id
                  ? "bg-[var(--color-accent-primary)] text-[#0C1220]"
        ...
        {totalSelected > 0 && (
          <div className="fixed bottom-24 right-6 z-40 animate-scale-in">
            <div className="flex items-center gap-3 px-5 py-3 rounded-2xl bg-gradient-to-l from-[var(--color-accent-primary)] to-[var(--color-accent-tertiary)] shadow-xl shadow-[var(--color-accent-primary)]/20">
              <div className="text-sm font-bold text-[#0C1220]">{totalSelected} בחירות</div>
              <button
                onClick={() => setSelectedOdds({})}
                className="text-xs font-medium text-[#0C1220]/70 hover:text-[#0C1220] transition-colors"
              >
                נקה
              </button>
            </div>
