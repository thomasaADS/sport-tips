...
export default function TipCard({ tip }: { tip: Tip }) {
  const confColor =
    tip.confidence >= 80 ? "emerald" : tip.confidence >= 65 ? "amber" : "blue";
  const confGradient =
    tip.confidence >= 80
    ...
    <div className="group rounded-2xl bg-[var(--color-bg-card)] border border-[var(--color-border-subtle)] hover:border-amber-500/25 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(245,158,11,0.08)] overflow-hidden">
      ...
      {tip.isHot && (
        <div className="bg-gradient-to-l from-amber-500 via-amber-500 to-orange-500 px-4 py-1.5 flex items-center justify-between">
          <span className="text-[11px] font-extrabold text-black tracking-wide">🔥 המלצה חמה</span>
          <span className="text-[10px] font-black text-black/70 tracking-widest">TOP PICK</span>
        </div>
      )}
      ...
      <div className="p-4">
        ...
        {/* Teams */}
        <div className="flex items-center gap-3 mb-4 py-3 px-3 rounded-xl bg-[var(--color-bg-secondary)]/80 border border-[var(--color-border-subtle)]">
          <div className="flex-1 text-center">
            <div className="w-10 h-10 mx-auto rounded-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/10 flex items-center justify-center text-base font-black text-amber-400 mb-1.5">
          ...
          <div className="flex-1 text-center">
            <div className="w-10 h-10 mx-auto rounded-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/10 flex items-center justify-center text-base font-black text-blue-400 mb-1.5">
        ...
        <div className="flex items-center gap-2.5 mb-3">
          <div className="flex-1 min-w-0 py-2.5 px-3 rounded-xl bg-[var(--color-bg-secondary)]/60 border border-[var(--color-border-subtle)]">
            <div className="text-[9px] text-[var(--color-text-dim)] font-semibold uppercase tracking-wider mb-0.5">המלצה</div>
            <div className="text-[13px] font-bold text-white truncate">{tip.recommendation}</div>
          </div>
          <div className={`px-4 py-2.5 rounded-xl bg-gradient-to-br ${confGradient} text-center flex-shrink-0`}>
